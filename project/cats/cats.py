...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def select(target):
        word_list = split(remove_punctuation(lower(target)))
        for word in topic:
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        min_diff = limit + 1
        most_related = user_word
        for valid in valid_words:
            current_diff = diff_function(user_word, valid, limit)
            if current_diff < min_diff and current_diff <= limit:
                min_diff = diff_function(user_word, valid, limit)
                most_related = valid
        return most_related
    ...
    for i in range(len(words)):
        ...
        for j in range(1, len(players)):
            if time(game, j, i) < min:
                min = time(game, j, i)
                min_index = j
        fastest[min_index] += [word_at(game, i)]
    return fastest
    # END PROBLEM 10

...
def balanced(m):
    ...
    condition2 = balanced(end(left(m))) and balanced(end(right(m)))
...
def totals_tree(m):
    ...
    "*** YOUR CODE HERE ***"
    if is_mobile(m):
        son_list = [totals_tree(end(left(m))), totals_tree(end(right(m)))]
        return tree(total_weight(m), son_list)
    else:
        return tree(size(m))
...
def has_path(t, phrase):
    ...
    if True in [has_path(son, phrase[1:]) for son in branches(t)] and label(t) == phrase[0]:
        return True
    else:
        return False

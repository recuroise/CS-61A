...
class VendingMachine:
    ...
    def add_funds(self, dollor):
        if self.num == 0:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(dollor)
        else:
            self.fund += dollor
...
def is_bst(t):
    ...
    def max_node(t):
        ...
        if len(t.branches) == 1:
            if t.label > t.branches[0].label:
                ...
                return max_node(t.branches[0])
        if len(t.branches) == 2:
            return max_node(t.branches[1])
    ...
    if t.is_leaf():
        ...
        if t.branches[0].label < t.label:
            left = t.branches[0]
            return is_bst(left) and max_node(left) <= t.label
        else:
            right = t.branches[0]
            return is_bst(right) and  t.label <= min_node(right)
    elif len(t.branches) == 2:
        left = t.branches[0]
        right = t.branches[1]
        return is_bst(left) and is_bst(right) and max_node(left) <= t.label <= min_node(right)
    else:
        return False
...
def store_digits(n):
    ...
    def link_extend(s, t):
        if s == Link.empty:
            return t
        return Link(s.first, link_extend(s.rest, t))
    if n < 10:
        return Link(n)
    s = store_digits(n // 10)
    return link_extend(s, Link(n % 10))
...
def deep_map(f, link):
    ...
    def copy(link):
        ...
        if type(link.first) != Link:
            ...
            return Link(copy(link.first), copy(link.rest))

    q = p = copy(link)
    while p != Link.empty:
         
        if type(p.first) != Link:
            p.first = f(p.first)
        else:
            p.first = deep_map(f, p.first)
